...
class Calc {
	public int findKthSmallest(List<Integer[]> lists, int k) {
		...
		//first of all populate all first elements 
		...
		// we poll() min element and move index forward for that removed element and add into heap
		while (counter <= k) {
			Node node = heap.poll();
			...
			// if counter matches [k] - we are done
			if (counter == k) {
				return currValue;
			} else {
